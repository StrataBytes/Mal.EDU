...
lock_file_path = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'maledu.lock')
...
if not have_lock:
    print("Another instance of Mal.EDU is running. Exiting...")
    sys.exit(0)
...
try:
    ...
    NUM_QUESTIONS = min(len(QUESTIONS), random.randint(7, 8)) #You can change it to something like 7 - 12
    ...
    timer_count = 110  #quiz timer. set to 55 s.
    ...
    def main_app():
        global root, question_frame, timer_label, status_label, question_counter_label
        root = tk.Tk()
        root.title("csrss")
    ...
    # Release the lock when the application exits or finishes
    lock.release()